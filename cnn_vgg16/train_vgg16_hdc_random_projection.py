# Modified for VGG16 feature extraction + Random Projection HDC classification with comprehensive metrics
...
for run_num in range(1, NUM_RUNS + 1):
    ...
    run_metrics = {
        ...
        'system_info': {
            ...
            'model_type': 'VGG16_RandomProjection'
        }
    }
    ...
    run_metrics['energy_consumment']['training']['cpu_kwh'] = training_cpu_energy
    run_metrics['energy_consumment']['training']['gpu_kwh'] = training_gpu_energy
    run_metrics['energy_consumment']['training']['total_kwh'] = total_training_energy
    run_metrics['energy_consumment']['testing']['cpu_kwh'] = testing_cpu_energy
    run_metrics['energy_consumment']['testing']['gpu_kwh'] = testing_gpu_energy
    run_metrics['energy_consumment']['testing']['total_kwh'] = total_testing_energy
...
total_energies = [run['energy_consumment']['training']['total_kwh'] + 
                 run['energy_consumment']['testing']['total_kwh'] for run in all_results]
...
output_file = f'vgg16_random_projection_metrics_{NUM_RUNS}_runs_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
final_output = {
    'experiment_info': {
        'total_runs': NUM_RUNS,

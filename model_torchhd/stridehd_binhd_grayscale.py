...
# Define the transform with Grayscale at the end as you specified
...
print(f"Number of channels: {num_channels} (grayscale)")
print(f"Number of classes: {num_classes}")


# =============================================================================
# StrideHD Encoder (Optimized for Grayscale)
# =============================================================================
class StrideHDEncoder(nn.Module):
    def __init__(self, in_channels, out_dims, window_size, stride, binary_levels, num_training_subsets, device):
        ...
        # For grayscale, we adjust the range since we have different feature dimensions
        self.register_buffer('R', torch.randint(0, 1500, (self.M, self.B)))
        
    def window_striding_maxpool(self, x):
        """Apply window striding and max-pooling for grayscale images"""
        ...
        # Extract patches - grayscale has only 1 channel, so simpler processing
        ...
        max_pooled = F.adaptive_max_pool2d(patches, (1, 1)).squeeze(-1).squeeze(-1)
        # max_pooled shape: [batch_size, total_patches]
        
        return max_pooled, h_patches * w_patches

    def thermometer_binarize(self, x):
        """Proper thermometer encoding for normalized grayscale values"""
        batch_size, num_features = x.shape
        
        # Since images are already normalized, we can use the existing range
        # But we need to handle negative values from normalization
    ...
    def pseudo_random_encoding(self, binary_patterns):
        """Paper's pseudo-random encoding"""
    ...
    def encode(self, x):
        """Main encoding function for grayscale images"""
...
# =============================================================================
# StrideHD Classifier (Same as before)
# =============================================================================
...
# Initialize encoder for grayscale (1 channel)
...
# Note: We apply grayscale conversion in the main transform, not here
...
print("Encoding grayscale datasets...")
...
print(f"Encoded grayscale dataset size - Training: {len(train_ds_encoded)}, Test: {len(test_ds_encoded)}")
...
print(f"\n{'='*60}")
print(f"STARTING EXPERIMENTS FOR STRIDEHD (GRAYSCALE)")
print(f"{'='*60}")
...
for run_num in range(1, NUM_RUNS + 1):
    ...
    print(f"\nStrideHD (Grayscale) - RUN {run_num} RESULTS:")
...
# Save results
output_file = f'stridehd_grayscale_metrics_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
final_output = {
    'experiment_info': {
        'total_classifiers': 1, 

...
NUM_EPOCHS = 40  # Same as original Keras model
LEARNING_RATE = 1e-3  # Same as original Keras model
BATCH_SIZE = 32  # Same as original Keras model

# CNN expects 256x256 images (same as original Keras model)
IMG_WIDTH = 256
IMG_HEIGHT = 256

# CNN preprocessing - matches original Keras implementation
# Keras uses: Rescaling(1.0/255) + RandomHorizontalFlip + RandomRotation(0.1 radians)
# PyTorch equivalent: ToTensor() scales to [0,1] + RandomHorizontalFlip + RandomRotation(5.73 degrees)
train_transforms = transforms.Compose([
    transforms.Resize((IMG_HEIGHT, IMG_WIDTH)),
    transforms.RandomHorizontalFlip(),
    transforms.RandomRotation(degrees=5.73),  # 0.1 radians = 5.73 degrees
    transforms.ToTensor(),  # This scales pixels to [0,1] like Keras Rescaling
])

test_transforms = transforms.Compose([
    transforms.Resize((IMG_HEIGHT, IMG_WIDTH)),
    transforms.ToTensor(),  # Only rescaling, no augmentation for testing
])
...
def main():
    """
    Main training loop for CNN with comprehensive metrics.
    """
    ...
    # ----------- GETS DATA --------------
    # Load the entire dataset using ImageFolder with separate training and test directories
    ...
    output_file = f'cnn_training_metrics_{NUM_EPOCHS}_epochs_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
    final_output = {
        'experiment_info': {

# Modified for Custom CNN feature extraction + NeuralHD HDC classification with comprehensive metrics
...
IMG_HEIGHT = 256
BATCH_SIZE = 32
NUM_EPOCHS = 40  # Match original CNN implementation
...
class NeuralHD(nn.Module):
    r"""Implements Scalable edge-based hyperdimensional learning system with brain-like neural adaptation.
    ...
    Args:
        ...
        device (``torch.device``, optional):  the desired device of the weights.
        dtype (``torch.dtype``, optional): the desired data type of the weights.
    """
    ...
    def fit(self, input: Tensor, target: Tensor):
        ...
        for epoch_idx in trange(1, self.epochs, desc="fit", disable=True):
            encoded = self.encoder(input)
...
for run_num in range(1, NUM_RUNS + 1):
    ...
    run_metrics = {
        ...
        'system_info': {
            ...
            'model_type': 'CustomCNN_NeuralHD'
        }
    }
    
    # Create and train model
    epochs = NUM_EPOCHS
    regen_freq = 10  # Adjusted for 40 epochs (regenerate every 10 epochs)
...
output_file = f'custom_cnn_neuralhd_metrics_{NUM_RUNS}_runs_{NUM_EPOCHS}_epochs_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
final_output = {
    'experiment_info': {
        'total_runs': NUM_RUNS,
